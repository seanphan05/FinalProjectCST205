...
# Making copied of the test data instead of modifying the original ones
...
def get_cat_dog_url_and_prices(size: int, use_test_data=True) -> list(list()):
    """
    This function will get us the URL's containing the animal images, some random USD value amount, 
    a name for the animal, and what the animal is worth in crypto from the random USD amount.

    *Note*: The random USD amount is never larger than 999 dollars
    ...
    for rand_val in np.random.choice([0,1], size=(size)):
        ...
        if rand_val == 0: 
            if use_test_data: # For testing purposes
                num_random_choice_left = len(cat_test_data_copy) # Get the total number of random animals in the testing data
                random_test_choice = cat_test_data_copy[np.random.randint(num_random_choice_left)] # Get a random animal 
                animal_url = random_test_choice[0].get('url') # Get it's image URL
            else:
        ...
        # Converting the random USD number to crptyo
...
if __name__ == '__main__':
    ...
    results = get_cat_dog_url_and_prices(1)
    print(results)
